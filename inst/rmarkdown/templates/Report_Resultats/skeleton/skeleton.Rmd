...
conductor_iris<-data.frame(camp=names(iris),
                           descripcio=c("Longitud de sepal","Amplada de sepal","longitul del petal","amplada del petal","Especies varies"),
                           taula_basal=c(0,1,2,3,0),recode=c("5","3.1/3.4","","",""))
...
# Table1  ------------
table1::table1(~ Species+Sepal.Length | Species , data = iris,overall="Total")

formu<-formula_table1("taula_basal",y="Species",taulavariables=conductor_iris,dt=iris)
table1::table1(formu, data = iris,overall=T,render.continuos=c(.="Mean(CV%)",.="Median"))

formu<-formula_table1("taula_basal",taulavariables=conductor_iris)
table1::table1(formu, data = iris,overall="Total")
